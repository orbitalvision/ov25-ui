...
They are hidden from layout and assistive technology by default. To opt in, add this to the configurator's custom styling (inside its shadow root when applicable):
...
Deployment requires the matching OV25 producer and this UI build. Either side can ship first; older clients ignore the additional field and older producers simply have no summaries. This change does not publish a package or opt a storefront into visible prices.

...
export function getProductGalleryImages(metadata: {
...
} | null | undefined, opts?: { cutoutFirst?: boolean; includeCutout?: boolean }): ProductImageInput[] {
  if (!metadata) return []
  ...
  const includeCutout = opts?.includeCutout !== false
  if (opts?.cutoutFirst && includeCutout && cutout && hero) {
    return [cutout, hero, ...gallery]
  }
  return [
    ...(hero ? [hero] : []),
    ...(includeCutout && cutout ? [cutout] : []),
    ...gallery,
  ]
}

export function getProductCutoutImage(
  metadata: Parameters<typeof getProductGalleryImages>[0],
): ProductImageInput | null {
  return toImageInput(metadata?.cutoutImage)
}

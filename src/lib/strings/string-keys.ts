...
export const STRING_REPLACEMENT_DEFINITIONS: StringReplacements = [
  ...
  { key: 'swatchesContainerOrderMaxFreeSwatches', label: 'Swatches container order N free swatches when max equals free limit', defaultTemplate: '+ Order ${MAX_SWATCHES} Free Swatches', interpolationValues: [{ name: 'MAX_SWATCHES', description: 'Maximum swatch count (same as the free swatch limit in this state).' }] },
  { key: 'swatchesContainerOrderUpToMaxSwatches', label: 'Swatches container order up to N swatches', defaultTemplate: '+ Order up to ${MAX_SWATCHES} Swatches', interpolationValues: [{ name: 'MAX_SWATCHES', description: 'Maximum total swatch count including paid selections.' }] },
